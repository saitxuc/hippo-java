...
import static com.pinganfu.hippo.leveldb.impl.LogMonitors.throwExceptionMonitor;
...
import com.pinganfu.hippo.leveldb.cluster.Ddb.LruLinkedHashMap;
import com.pinganfu.hippo.leveldb.table.UserComparator;
import com.pinganfu.hippo.leveldb.util.InternalIterator;
import com.pinganfu.hippo.leveldb.util.Level0Iterator;
...
public class VersionSet implements SeekingIterable<InternalKey, Slice>
{
    ...
    public static final int TARGET_FILE_SIZE = Options.TARGET_FILE_SIZE;
    ...
    /**
     * original is <code>new MapMaker().weakKeys().makeMap()</code></br>
     * 使用了weak引用，用来防止version长时间没有没释放，
     * 在version持续频繁的变化时，存在被retain的version发生gc可能性，会导致slave再次重新拉取，</br>
     * 现在采用lru的方式，避免某个version长时间没锁的，而没有被释放。
     */
    private final Map<Version, Object> activeVersions = new LruLinkedHashMap<Version, Object>(16);
...
//        }
		...
		Compaction compaction = new Compaction(current, level, levelInputs, levelUpInputs, grandparents).setKeyRange(Maps
				.immutableEntry(smallest, largest));
    ...
    private static class Builder
    {
        ...
        public void apply(VersionEdit edit)
        {
            ...
            // Delete bFiles
            for (Entry<Integer, BucketMetaData> entry : edit.getDeletedBFiles().entries()) {
                Integer level = entry.getKey();
                BucketMetaData bucketMetaData = entry.getValue();
                levels.get(level).deletedBFiles.add(bucketMetaData);
            }
            
            // Add new bfiles
	        for (Entry<Integer, BucketMetaData> entry : edit.getNewBFiles().entries()) {
	            Integer level = entry.getKey();
	            BucketMetaData bucketMetaData = entry.getValue();
	            if (!levels.get(level).deletedFiles.contains(bucketMetaData.getFileNumber())) {
	            	levels.get(level).deletedBFiles.remove(bucketMetaData);
	            	levels.get(level).addedBFiles.add(bucketMetaData);
	            } else {
	            	 levels.get(level).deletedBFiles.add(bucketMetaData);
	            }
	        }
                ...
                throws IOException
        {
            ...
            for (int level = 0; level < baseVersion.numberOfLevels(); level++) {
                ...
                //----------bucket----
                Collection<BucketMetaData> baseBFiles = baseVersion.getBFiles().asMap().get(level);
				if (baseBFiles == null) {
					baseBFiles = ImmutableList.of();
				}
				Set<BucketMetaData> addedBFiles = levels.get(level).addedBFiles;
				if (addedBFiles == null) {
					addedBFiles = ImmutableSortedSet.of();
				}

				ArrayList<BucketMetaData> bFiles = newArrayListWithCapacity(baseBFiles.size() + addedBFiles.size());
				bFiles.addAll(baseBFiles);
				bFiles.addAll(addedBFiles);

				for (BucketMetaData bucketMetaData : bFiles) {
					maybeAddBfile(version, level, bucketMetaData);
				}
            }
        }
        
		private void maybeAddBfile(Version version, int level, BucketMetaData bucketMetaData) throws IOException {
			if (levels.get(level).deletedBFiles.contains(bucketMetaData)) {
				System.out.println(bucketMetaData);
				// File is deleted: do nothing
			} else {
				version.addBFile(bucketMetaData);
			}
		}
        ...
        private static class LevelState
        {
            ...
            private final Set<BucketMetaData> addedBFiles;
            private final Set<BucketMetaData> deletedBFiles;
            ...
            public LevelState(InternalKeyComparator internalKeyComparator)
            {
                ...
                addedBFiles = new HashSet<BucketMetaData>();
                deletedBFiles = new HashSet<BucketMetaData>();
            }
     ...
     * 两个key range是否存在交集
     */
	public boolean overlap(Entry<InternalKey, InternalKey> range0, Entry<InternalKey, InternalKey> range1) {
		if (range0 == null || range1 == null) {
			return false;
		}
		// range0'smallest in range1 or range0'largest in range1
		return (getInternalKeyComparator().compare(range0.getKey(), range1.getKey()) >= 0 && getInternalKeyComparator().compare(
				range0.getKey(), range1.getValue()) <= 0)
				|| (getInternalKeyComparator().compare(range0.getValue(), range1.getKey()) >= 0 && getInternalKeyComparator()
						.compare(range0.getValue(), range1.getValue()) <= 0);
	}
	
	public Set<Version> activeVersions() {
		return activeVersions.keySet();
	}

...
import static com.hippo.leveldb.impl.FileMetaData.GET_LARGEST_USER_KEY;
    ...
    public LookupResult get(LookupKey key, ReadStats readStats)
    {
        ...
        for (FileMetaData fileMetaData : fileMetaDataList) {
            ...
            if (iterator.hasNext()) {
            	...
            	if (key.getUserKey().equals(internalKey.simplify().getUserKey())) {
					if (key.getVersion() == 0 || internalKey.version() == key.getVersion()) {
						if (internalKey.getValueType() == ValueType.DELETION) {
							return LookupResult.deleted(key);
						} else if (internalKey.getValueType() == VALUE && !internalKey.expire()) {
							return LookupResult.ok(key, entry.getValue()).expireTime(internalKey.expireTime()).version(internalKey.version());
						}
					}
                }
            }

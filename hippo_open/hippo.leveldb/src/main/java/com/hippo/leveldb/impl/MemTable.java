...
public class MemTable implements SeekingIterable<InternalKey, Slice>
{
    ...
    public LookupResult get(LookupKey key)
    {
        ...
        Entry<InternalKey, Slice> entry = table.ceilingEntry(internalKey.simplify());
        if (entry == null) {
            return null;
        }

        InternalKey entryKey = entry.getKey();
		if (entryKey.simplify().getUserKey().equals(key.getUserKey())) {
			if (key.getVersion() == 0 || entryKey.version() == key.getVersion()) {
				if (entryKey.getValueType() == ValueType.DELETION) {
					return LookupResult.deleted(key);
				} else if (!entryKey.expire()) {
					return LookupResult.ok(key, entry.getValue()).expireTime(entryKey.expireTime()).version(entryKey.version());
				}
			}
		}
        return null;

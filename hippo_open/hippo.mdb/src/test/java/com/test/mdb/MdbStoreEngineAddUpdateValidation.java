            ...
            public void run() {
                try {
                    for (int i = 1; i <= 50; i++) {
                        ...
                        for (int count = 1; count <= 10000; count++) {
                            String key = "k" + (count + (10000 * (i - 1)));
                            String key2 = "k" + (count + (10000 * (i - 1)))+"-update";
                            boolean result2 = storeEngine.updateData(key.getBytes(), key2.getBytes(), 1000,0);
                            if (!result2) {
                                System.out.println("data added expcetion!");
                            }
                        }
                ...
                for (int i = 1; i <= 500000; i++) {
                    ...
                    try {
                        ...
                        if (!(key+"-update").equals(new String(result.getContent()))) {

...
public class TestObject implements Serializable {
    ...
    public static void main(String[] args) throws HippoStoreException {
//		final StoreEngine engine = new MdbStoreEngine();
//		engine.start();
//		
//		engine.addData("1", "************", 100000);
////		engine.addData("2", "***********", 1000000);
//		engine.addData("2", "*************", 1000000);
//		
//		engine.getData("2");
//		engine.stop();
        ...
        String s1 = "message body,FD 和 Type 列的含义最为模糊，它们提供了关于文件如何使用的更多信息。"
        ...
        String s2 = "message body,FD 和 Type 列的含义最为模糊，它们提供了关于文件如何使用的更多信息。"
                + "FD 列表示文件描述符，应用程序通过文件描述符识别该文件。Type 列提供了关于文件格"
                + "式的更多描述。我们来具体研究一下文件描述符列，清单 1 中出现了三种不同的值。cwd 值表示"
                + "应用程序的当前工作目录，这是该应用程序启动的目录，除非它本身对这个目录进行更改。txt 类型的文件是程序代码"
                + "，如应用程序二进制文件本身或共享库，再比如本示例的列表中显示的 init 程序。最后，数值表示应用程序的文件描述符，"
        ...
        //	int t1 = sun.misc.Hashing.murmur3_32(a1);
        //	int t2 = sun.misc.Hashing.murmur3_32(a2);

...
import com.hippo.redis.BulkReply;
import com.hippo.redis.ErrorReply;
import com.hippo.redis.IntegerReply;
import com.hippo.redis.MultiBulkReply;
import com.hippo.redis.Reply;
import com.hippo.redis.StatusReply;
...
public class RedisReplyDecoder extends ReplayingDecoder<Void> {
	...
	protected void decode(ChannelHandlerContext channelHandlerContext,
			ByteBuf byteBuf, List<Object> objects) throws Exception {
	...
	public Reply readReply(ByteBuf is) throws IOException {
		...
		case StatusReply.MARKER: {
			String status = is.readBytes(is.bytesBefore((byte) '\r')).toString(
					Charsets.UTF_8);
			is.skipBytes(2);
			return new StatusReply(status);
		}
		case ErrorReply.MARKER: {
			String error = is.readBytes(is.bytesBefore((byte) '\r')).toString(
					Charsets.UTF_8);
			is.skipBytes(2);

...
public class SerializerFactory extends AbstractSerializerFactory {
	...
	public Serializer getSerializer(Class cl) throws HessianProtocolException {
		...
		for (int i = 0; serializer == null && _factories != null
				&& i < _factories.size(); i++) {
	...
	public Object readObject(AbstractHessianInput in, String type,
			String[] fieldNames) throws HessianProtocolException, IOException {
		Deserializer deserializer = getDeserializer(type);
	...
	public Deserializer getDeserializer(String type)
		...
		if (type.startsWith("[")) {
			...
			try {
				...
				log.warning("Hessian/Burlap: '" + type
						+ "' is an unknown class in " + _loader + ":\n" + e);
	...
	static {
		...
		_staticDeserializerMap.put(Number.class, new BasicDeserializer(
				BasicSerializer.NUMBER));
		...
		try {
			_staticDeserializerMap.put(File.class, new StringValueDeserializer(
					File.class));
		} catch (Throwable e) {
		}
		...
		_staticSerializerMap.put(java.sql.Timestamp.class,
		...
		try {
			...
			_staticDeserializerMap.put(stackTrace,

...
public class ZipMap {
    public static final int ZIPMAP_BIGLEN = 254; //zipMap start
    public static final int ZIPMAP_END = 255; //zipMap end
    ...
    static int zipmapDecodeLength(byte[] p, int start) {
        ...
        len = ((p[start + 4] & 0x00ff) << 24) + ((p[start + 3] & 0x00ff) << 16) + ((p[start + 2] & 0x00ff) << 8) + ((p[start + 1] & 0x00ff));
        return len;
    }

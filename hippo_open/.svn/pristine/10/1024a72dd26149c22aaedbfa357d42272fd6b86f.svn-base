 ...
 *
 */
public class OffHeapMapTest<K, Pointer> extends AbstractMap<K, Pointer> implements ConcurrentMap<K, Pointer>, Serializable {
    /**
     * 
     */
    ...
    public OffHeapMapTest(int initialCapacity, float loadFactor, int concurrencyLevel, MdbManager mdbManager) {
        ...
        Segment<K, Pointer> s0 = new Segment<K, Pointer>(loadFactor, (int) (cap * loadFactor), (HashEntry<K, Pointer>[]) new HashEntry[cap], this.mdbManager);
     ...
     * @since 1.6
     */
    public OffHeapMapTest(int initialCapacity, float loadFactor) {
     ...
     */
    public OffHeapMapTest(int initialCapacity) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, null);
    }

    public OffHeapMapTest(MdbManager mdbManager) {
    ...
    public void putAll(Map<? extends K, ? extends Pointer> m) {
        for (Entry<? extends K, ? extends Pointer> e : m.entrySet())
    ...
    public Pointer get(Object key) {
        ...
        HashEntry<K, Pointer>[] tab;
        int h = hash(key);
        long u = (((h >>> segmentShift) & segmentMask) << SSHIFT) + SBASE;
        if ((s = (Segment<K, Pointer>) UNSAFE.getObjectVolatile(segments, u)) != null && (tab = s.table) != null) {
            for (HashEntry<K, Pointer> e = (HashEntry<K, Pointer>) UNSAFE.getObjectVolatile(tab, ((long) (((tab.length - 1) & h)) << TSHIFT) + TBASE); e != null; e = e.next) {
    ...
    transient Set<Entry<K, Pointer>> entrySet;

    @Override
    public Set<Entry<K, Pointer>> entrySet() {
        Set<Entry<K, Pointer>> es = entrySet;
    ...
    private Segment<K, Pointer> ensureSegment(int k) {
        ...
        if ((seg = (Segment<K, Pointer>) UNSAFE.getObjectVolatile(ss, u)) == null) {
            ...
            HashEntry<K, Pointer>[] tab = (HashEntry<K, Pointer>[]) new HashEntry[cap];
    ...
    static final class Segment<K, Pointer> extends ReentrantLock implements Serializable {
         ...
         */
        transient volatile HashEntry<K, Pointer>[] table;
        ...
        Segment(float lf, int threshold, HashEntry<K, Pointer>[] tab, MdbManager mdbManager) {
            this.loadFactor = lf;
        ...
        final Pointer put(K key, int hash, Pointer value, boolean onlyIfAbsent) {
            HashEntry<K, Pointer> node = tryLock() ? null : scanAndLockForPut(key, hash, value);
            Pointer oldValue;
            try {
                HashEntry<K, Pointer>[] tab = table;
                int index = (tab.length - 1) & hash;
                HashEntry<K, Pointer> first = entryAt(tab, index);
                for (HashEntry<K, Pointer> e = first;;) {
                    if (e != null) {
                        ...
                        if (node != null)
                            ...
                            node = new HashEntry<K, Pointer>(hash, key, value, first);
                        int c = count + 1;
         ...
         */
        @SuppressWarnings("unchecked")
        private void rehash(HashEntry<K, Pointer> node) {
            /*
             ...
             * table write.
             */
            HashEntry<K, Pointer>[] oldTable = table;
            int oldCapacity = oldTable.length;
            int newCapacity = oldCapacity << 1;
            threshold = (int) (newCapacity * loadFactor);
            HashEntry<K, Pointer>[] newTable = (HashEntry<K, Pointer>[]) new HashEntry[newCapacity];
            int sizeMask = newCapacity - 1;
            for (int i = 0; i < oldCapacity; i++) {
                HashEntry<K, Pointer> e = oldTable[i];
                if (e != null) {
                    HashEntry<K, Pointer> next = e.next;
                    int idx = e.hash & sizeMask;
                    if (next == null) //  Single node on list
                        newTable[idx] = e;
                    else { // Reuse consecutive sequence at same slot
                        HashEntry<K, Pointer> lastRun = e;
                        int lastIdx = idx;
                        for (HashEntry<K, Pointer> last = next; last != null; last = last.next) {
                        ...
                        for (HashEntry<K, Pointer> p = e; p != lastRun; p = p.next) {
                            Pointer v = p.value;
                            int h = p.hash;
                            K key = p.key;
                            int k = h & sizeMask;
                            HashEntry<K, Pointer> n = newTable[k];
                            newTable[k] = new HashEntry<K, Pointer>(h, key, v, n);
                        }
                    }
                }
            }
         ...
         */
        private HashEntry<K, Pointer> scanAndLockForPut(K key, int hash, Pointer value) {
            HashEntry<K, Pointer> first = entryForHash(this, hash);
            HashEntry<K, Pointer> e = first;
            HashEntry<K, Pointer> node = null;
            int retries = -1; // negative while locating node
            while (!tryLock()) {
                HashEntry<K, Pointer> f; // to recheck first below
                if (retries < 0) {
                    if (e == null) {
                        if (node == null) // speculatively create node
                            node = new HashEntry<K, Pointer>(hash, key, value, null);
                        retries = 0;
                    } else {
                        MdbPointer mp = (MdbPointer) e.value;
                        byte[] nkey = (byte[]) key;
        ...
        final Pointer remove(Object key, int hash, Object value) {
            ...
            try {
                HashEntry<K, Pointer>[] tab = table;
                int index = (tab.length - 1) & hash;
                HashEntry<K, Pointer> e = entryAt(tab, index);
                HashEntry<K, Pointer> pred = null;
                while (e != null) {
                    HashEntry<K, Pointer> next = e.next;
    ...
    static final <K, V> HashEntry<K, V> entryForHash(Segment<K, V> seg, int h) {
        HashEntry<K, V>[] tab;
        return (seg == null || (tab = seg.table) == null) ? null : (HashEntry<K, V>) UNSAFE
            .getObjectVolatile(tab, ((long) (((tab.length - 1) & h)) << TSHIFT) + TBASE);
    }

    static final class HashEntry<K, Pointer> {
        final int hash;
        volatile K key;
        volatile Pointer value;
        volatile HashEntry<K, Pointer> next;

        HashEntry(int hash, K key, Pointer value, HashEntry<K, Pointer> next) {
            this.key = key;
            this.hash = hash;
            this.value = value;
            this.next = next;
         ...
         */
        final void setNext(HashEntry<K, Pointer> n) {
    ...
    final class EntrySet extends AbstractSet<Entry<K, Pointer>> {
        public Iterator<Entry<K, Pointer>> iterator() {
            return new EntryIterator();
        }

        public boolean contains(Object o) {
            throw new UnsupportedOperationException("contains(Object value) | This method not suppouted in this version!");
        }
        ...
        public boolean remove(Object o) {
            if (!(o instanceof Entry))
                return false;
            Entry<?, ?> e = (Entry<?, ?>) o;
            return OffHeapMapTest.this.remove(e.getKey(), e.getValue());
        }

        public int size() {
            return OffHeapMapTest.this.size();
        }

        public boolean isEmpty() {
            return OffHeapMapTest.this.isEmpty();
        }

        public void clear() {
            OffHeapMapTest.this.clear();
        }
    }

    final class EntryIterator extends HashIterator implements Iterator<Entry<K, Pointer>> {
        public Entry<K, Pointer> next() {
            HashEntry<K, Pointer> e = super.nextEntry();
    ...
    abstract class HashIterator {
        ...
        int nextTableIndex;
        HashEntry<K, Pointer>[] currentTable;
        HashEntry<K, Pointer> nextEntry;
        HashEntry<K, Pointer> lastReturned;
        ...
        final void advance() {
            for (;;) {
                if (nextTableIndex >= 0) {
                    ...
                    Segment<K, Pointer> seg = segmentAt(segments, nextSegmentIndex--);
        ...
        final HashEntry<K, Pointer> nextEntry() {
            HashEntry<K, Pointer> e = nextEntry;
            if (e == null)
        ...
        public final void remove() {
            ...
            OffHeapMapTest.this.remove(lastReturned.key);
            lastReturned = null;
        }
    }

    final class WriteThroughEntry extends SimpleEntry<K, Pointer> {
        WriteThroughEntry(K k, Pointer v) {
            super(k, v);
        }
        ...
        public Pointer setValue(Pointer value) {
            if (value == null)
                throw new NullPointerException();
            Pointer v = super.setValue(value);
            OffHeapMapTest.this.put(getKey(), value);
            return v;
        }
    }
}

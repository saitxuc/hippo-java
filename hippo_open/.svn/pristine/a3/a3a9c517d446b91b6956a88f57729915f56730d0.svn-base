    ...
    NEW_FILE(7)
            {
                ...
                public void readValue(SliceInput sliceInput, VersionEdit versionEdit)
                {
                    ...
                    // bucket number
                    int bucketSize = VariableLengthQuantity.readVariableLengthInt(sliceInput);
                    Set<Integer> buckets = Sets.newHashSetWithExpectedSize(bucketSize);
                    for (int i = 0; i < bucketSize; i++) {
                    	buckets.add(VariableLengthQuantity.readVariableLengthInt(sliceInput));
					}
                    versionEdit.addFile(level, fileNumber, fileSize, smallestKey, largestKey, buckets);
                }
                ...
                public void writeValue(SliceOutput sliceOutput, VersionEdit versionEdit)
                {
                    for (Entry<Integer, FileMetaData> entry : versionEdit.getNewFiles().entries()) {
                        ...
                        // bucket number
                        VariableLengthQuantity.writeVariableLengthInt(fileMetaData.getBuckets().size(), sliceOutput);
                        for (Integer b : fileMetaData.getBuckets()) {
                        	VariableLengthQuantity.writeVariableLengthInt(b, sliceOutput);
						}
                    }
                }
            };
